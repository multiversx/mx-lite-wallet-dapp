...
import { Button, SidePanel } from 'components';
...
const containerStyles = {
  ...
  modal: {
    padding: modalStyles.panel.padding,
    borderRadius: '8px',
    width: '100%',
    maxWidth: '100%'
  }
...
const Panel = ({ isOpen, onSubmit, onClose, anchor }: PanelProps) => {
  ...
  return anchor ? (
    createPortal(<div style={styles.overlay}>{content}</div>, anchor)
  ) : (
    <SidePanel
      isOpen={isOpen}
      ...
      root.render(
        <Panel
          ...
          onSubmit={(values) => {
            ...
            renderPanel(); // Re-render to close the panel
          }}
          onClose={() => {
            this._currentPanel.isOpen = false;
            if (this._currentPanel.resolve) {
