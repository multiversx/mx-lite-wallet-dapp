...
import { CreateRoutesEnum, DataTestIdsEnum } from 'localConstants';
...
export const CreateMnemonics = () => {
  ...
  const goToCheckMnemonic = () => {
    if (touched && isValid) {
      return pushAndNavigate(CreateRoutesEnum.checkMnemonic);
    }
  ...
  if (!createRecoverWalletRoutes.includes(CreateRoutesEnum.mnemonicPhrase)) {
    return <Navigate to={CreateRoutesEnum.info} />;
  }
